...
1 초	128 MB	3790	2010	1592	53.209%
문제
동근이는 무인 경비 회사 경비원으로 항상 대기하고 있다가 호출이 들어오면
경비차를 몰고 그 곳으로 달려가야 한다.
...
첫째 수는 상점이 위치한 방향을 나타내는데, 1은 블록의 북쪽, 2는 블록의 남쪽, 3은 블록의 서쪽, 4는 블록의 동쪽에
상점이 있음을 의미한다.
...
for i in range(len(locations)-1) :
    ...
    elif locations[-1][0] == 3 :

    elif locations[-1][0] == 2:

    elif locations[-1][0] == 1 :
