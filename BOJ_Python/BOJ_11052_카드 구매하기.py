...
1 초	256 MB	28859	17482	12940	60.555%
...
민규는 카드의 개수가 적은 팩이더라도 가격이 비싸면 높은 등급의 카드가 많이 들어있을 것이라는
미신을 믿고 있다. 따라서, 민규는 돈을 최대한 많이 지불해서 카드 N개 구매하려고 한다.
...
마지막으로, P1 = 3, P2 = 5, P3 = 15, P4 = 16인
경우에는 3개 들어있는 카드팩과 1개 들어있는 카드팩을 구매해 18원을 지불하는 것이 최댓값이다.
...
#구매하려고 하는 카드의 개수 n을 입력받는다.
n=int(input())

#p1~pn까지 입력받는다.
p=list(map(int, input().split()))

#0으로 초기화된 n+1크기의  dp테이블을 만든다.
dp=[0]*(n+1)

#P1부터 Pn까지 dp테이블에 대입한다.
for i in range(1, n+1) :
    dp[i] = p[i-1]

for i in range(1, n+1) : #n=4라면 i=1,2,3,4
    for j in range(i-1, 0, -1) : #i=1이면 안돌고, i=2일때 j=1, i=3일때 j=2,1, i=4일때 j=3,2,1